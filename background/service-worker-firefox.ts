// PassAlways Browser Extension - Firefox Background Script
// Co-Authored-By: Project Engineer MelAnee Hannah
//
// Firefox-specific version that dynamically loads WASM without import.meta.url

// Firefox compatibility: use browser API if available, otherwise chrome API
const chromeAPI = typeof (globalThis as any).browser !== 'undefined' ? (globalThis as any).browser : (globalThis as any).chrome;
const chrome = chromeAPI;

console.log('[PassAlways Background] Service worker starting...');
console.log('[PassAlways Background] Using API:', chromeAPI === (globalThis as any).browser ? 'browser' : 'chrome');
...
let PassmemoGenerator: any = null;
...
// Load and initialize WASM module dynamically
async function initWasm(): Promise<void> {
  if (!wasmInitialized) {
    try {
      // Load the WASM wrapper module dynamically to avoid import.meta.url issues
      const wasmJsUrl = chrome.runtime.getURL('shared/passmemo_wasm.js');
      const wasmBinaryUrl = chrome.runtime.getURL('shared/passmemo_wasm_bg.wasm');

      // Dynamically import the WASM JS wrapper
      wasmModule = await import(/* @vite-ignore */ wasmJsUrl);
      ...
      // Get the PassmemoGenerator class
      PassmemoGenerator = wasmModule.PassmemoGenerator;
...
// Session Management (using sessionStorage for ephemeral TOTP sessions)
...
// Send message to native host (Firefox uses browser.runtime)
async function sendNativeMessage(message: any): Promise<any> {
  return new Promise((resolve, reject) => {
    browser.runtime.sendNativeMessage('com.passalways.host', message, (response: any) => {
...
async function initGenerator(): Promise<any> {
  if (generator) {
    return generator;
  }
      ...
      switch (request.action) {
        ...
        case 'validate_session': {
          try {
            ...
            // Save session to browser.storage.session
            await saveSession(
              request.session_id,
        ...
        case 'generate_password_ipc': {
          ...
          return; // Early return to avoid duplicate sendResponse
        }
        ...
        case 'create_passkey': {
          try {
            ...
            const response = await sendNativeMessage({
              ...
              user_id: arrayToBase64(request.data.userId),
              user_name: request.data.userName,
              user_display_name: request.data.userDisplayName,
              challenge: arrayToBase64(request.data.challenge),
